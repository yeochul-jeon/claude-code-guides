...
1. [글로벌 CLAUDE.md 설정 현황](#1-글로벌-claudemd-설정-현황)
   - [Karpathy 4원칙 행동지침](#실전-예시-andrej-karpathy-4원칙--행동지침)
...
### 실전 예시: Andrej Karpathy 4원칙 — 행동지침

Andrej Karpathy의 LLM 코딩 관찰에서 출발해 커뮤니티가 정리한 4가지 행동지침
...
| **Simplicity First** | 요청한 것만, 추상화 최소 | Over-Engineering(과잉 복잡화) |
| **Surgical Changes** | 요청 부분만 건드리기 | [Shadow Agent](./claude-code-하네스-심화-아키텍처.md) (범위 초과 수정) |
| **Goal-Driven Execution** | 검증 가능한 성공 기준 → 반복 실행 | [Doom Loop](./claude-code-하네스-심화-아키텍처.md) (무한 재수정 루프) |

> Cotellese 스타일의 `Simple over clever`, `Ask, don't assume`과 철학이 같다. 아래 블록은 4원칙을 CLAUDE.md 규칙으로 구체화한 복붙용 버전이다.

```markdown
# ~/.claude/CLAUDE.md (Karpathy 4원칙 기반)

## Think Before Coding
- 가정은 반드시 명시한다. 불확실하면 실행 전 질문
- 여러 해석이 가능하면 모두 제시하고 선택을 요청 (침묵 금지)
- 더 간단한 방법이 있으면 언급한 뒤 진행 여부를 확인
- 불명확한 부분이 있으면 작업을 중단하고 구체적으로 질문

## Simplicity First
- 요청 범위를 벗어나는 기능은 추가하지 않는다
- 단일 용도 코드에 추상화 레이어를 만들지 않는다
- 200줄 코드가 50줄로 해결된다면 리팩터링 제안
- 자문: 시니어 엔지니어가 과도하다고 할까? → 그렇다면 단순화

## Surgical Changes
- 요청받은 부분 외에는 건드리지 않는다
- 기존 스타일이 마음에 들지 않더라도 그대로 따른다
- 관련 없는 데드 코드를 발견하면 삭제하지 말고 언급만
- 내 변경이 만든 미사용 import·변수만 제거

## Goal-Driven Execution
- 버그 수정 → 재현하는 테스트 작성 후 통과 확인
- 리팩터링 → 전후 테스트 동일하게 통과
- 다단계 작업은 각 단계에 검증 기준을 붙여서 계획:
  1. [단계] → 검증: [확인 방법]
  2. [단계] → 검증: [확인 방법]
```
...
CLAUDE.md를 모호한 가이드라인 대신 **결정론적 조건-행동 테이블**로 작성하면 준수율이 크게 높아진다.
...
**작성 원칙:**
- 조건은 측정 가능하게 ("좋은 경우" X, "30줄 초과" O)
- 행동은 구체적으로 ("적절히 처리" X, "try-catch 추가" O)
- 예외 없이 적용 (예외가 필요하면 규칙을 재설계)
...
이론적 기반: [하네스 엔지니어링 방법론 §7](claude-code-하네스-엔지니어링-방법론.md#7-if-then-규칙-엔진-패턴)
