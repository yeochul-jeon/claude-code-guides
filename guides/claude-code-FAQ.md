...
> 더 보기: [10단계 마스터리 §"2번 실패 → /clear" 규칙](./claude-code-10단계-마스터리-로드맵.md#2번-실패--clear-규칙)
...
### Q: Claude Code 기능을 쉽게 탐색하는 방법이 있나요?

`/powerup`을 실행하면 애니메이션 데모가 포함된 대화형 레슨으로 주요 기능을 단계별로 배울 수 있습니다.

```
/powerup
```

> 처음 Claude Code를 시작하거나 새 팀원 온보딩 시 가장 빠른 기능 파악 방법입니다.
...
**"작업 시작 전 요구사항이 불명확하다"** → AskUserQuestion 인터뷰 + SPEC.md 패턴으로 명세를 먼저 수집

> [하네스 엔지니어링 방법론 §10 — AskUserQuestion + SPEC.md](./claude-code-하네스-엔지니어링-방법론.md#10-사양-수집-패턴--askuserquestion--specmd): Claude에게 구현 전 인터뷰를 지시하는 메타프롬프트 + `.claude/SPEC.md` 단일 진실 소스 패턴
