...
9. [자동화 레벨 체계: 0 → 7](#9-자동화-레벨-체계-0--7)
...
### 4.1.1 Skill 유지보수: 벤치마킹 · Deprecation 추적

Skill은 만들기보다 **관리**가 어렵다. 방치된 Skill이 오래된 지식을 전파하거나 없는 경로를 참조하는 걸 막으려면 주기적 점검이 필요하다.

**월 1회 건강도 체크리스트:**

| 기준 | 행동 |
|------|------|
| 최근 1달 호출 0회 | 삭제 후보로 표시 |
| 참조 파일·명령어가 존재하지 않음 | 즉시 수정 또는 삭제 |
| 전제 도구(MCP/바이너리)가 없어짐 | SKILL.md 상단에 deprecation 주석 추가 |
| 더 간단한 방법이 생김 | 단순화 또는 다른 Skill과 병합 |

**Deprecation 패턴 — SKILL.md 상단에 추가:**

```markdown
<!-- DEPRECATED: 2026-07-01 이후 /new-deploy로 대체됨. 삭제 예정. -->
```

**벤치마킹 — Skill 효과 측정:**

"이 Skill이 없었다면 CLAUDE.md 규칙으로 남겼을까?" → Yes면 Skills가 맞는 위치다.

```bash
# work-log 기반 호출 빈도 확인
grep -r "/commit-helper\|/code-review" ~/.claude/projects/*/memory/ 2>/dev/null | wc -l
```
...
하네스 성숙도를 **8단계 레벨**로 측정한다. 레벨이 높을수록 반복 작업이 자동화되고, 사람은 의사결정에만 집중한다.

| 레벨 | 이름 | 구성 요소 | 특징 |
|------|------|----------|------|
| **0** | 수동 | 프롬프트만 | 매번 동일한 지시 반복 |
| **1** | 명세화 | CLAUDE.md 규칙 | 지시를 한 번 작성 → 세션마다 자동 적용 |
| **2** | 경계 설정 | permissions allow/deny | 위험 행동 원천 차단 |
| **3** | 강제화 | Hooks (PreToolUse/PostToolUse/Stop) | 100% 결정론적 가드레일 |
| **4** | 워크플로우화 | Skills + Commands | 반복 패턴을 재사용 가능한 레시피로 |
| **5** | 전문화 | Subagents (code-reviewer 등) | 특정 관점의 독립 리뷰 |
| **6** | 파이프라인화 | CI/CD (GitHub Actions) | 코드베이스와 AI 완전 통합 |
| **7** | 협업 | Agent Teams | 여러 Claude 인스턴스가 팀으로 작업 |

**레벨 진입 기준 — "이 상황이라면 다음 레벨 검토":**

```
레벨 0 → 1: 같은 지시를 3번 이상 반복했다
레벨 1 → 2: Claude가 .env 파일 수정을 시도한 적 있다
레벨 2 → 3: CLAUDE.md 규칙을 가끔 무시한다
레벨 3 → 4: 동일 워크플로우를 주 3회 이상 실행한다
레벨 4 → 5: 코드 리뷰 품질에 일관성이 필요하다
레벨 5 → 6: PR마다 수동으로 리뷰를 트리거한다
레벨 6 → 7: 하나의 작업을 여러 관점에서 동시에 분석해야 한다
```
...
| 레벨 | Phase |
|------|-------|
| 0 ~ 1 | Phase 1: 글로벌 기반 구축 |
| 1 ~ 2 | Phase 2: 프로젝트별 맞춤 구성 |
| 3 ~ 4 | Phase 3: 생산성 확장 |
| 4 ~ 5 | Phase 4: 팀 공유 및 표준화 |
| 6 ~ 7 | Phase 5: 자동화 및 CI/CD |

> 거버넌스 성숙도 5단계 비교: [팀 IDE 통합 가이드 §10](claude-code-팀-IDE-통합-가이드.md#10-보안-및-거버넌스)
