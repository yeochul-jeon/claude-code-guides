...
- [하네스 심화: 실패 패턴·참조 아키텍처·거버넌스](claude-code-하네스-심화-아키텍처.md) — 하네스 없이 쓸 때 생기는 4가지 실패 패턴
...
> **하네스 없이 그냥 쓰면 어떻게 될까?**
> AI Slop(틀린 코드 축적), Doom Loop(같은 오류 반복 수정), DB Mutation(데이터 파괴), Shadow Agent(요청 범위 초과) — 4가지 실패 패턴이 반복된다.
> 이 튜토리얼을 완료하면 모두 예방 가능하다. → [실패 패턴 상세](claude-code-하네스-심화-아키텍처.md#1-하네스-없을-때의-4가지-실패-패턴)
