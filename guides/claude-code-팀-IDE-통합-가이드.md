...
- [하네스 심화: 실패 패턴·참조 아키텍처·거버넌스](claude-code-하네스-심화-아키텍처.md) — 거버넌스 성숙도 5단계, Stripe 6계층 참조 아키텍처
...
팀의 현재 단계를 파악하고 다음 레벨로 나아갈 행동을 설정하라.

| 레벨 | 특징 | AI 비용 | 다음 단계 |
|------|------|---------|---------|
| **1 Chaos** | 팀원마다 다른 설정, 규칙 없음 | ~$7,000/월 | 팀 CLAUDE.md 공유 |
| **2 Awareness** | 공통 CLAUDE.md 존재, 강제 없음 | 높음 | settings.json + deny 규칙 배포 |
| **3 Standardization** | 공통 설정 강제 적용, Hooks 동작 | 절감 시작 | code-reviewer agent + Eval 세트 |
| **4 Optimization** | 독립 리뷰어, 회귀 테스트 운영 | 최적화 중 | 분기별 자동 하네스 재평가 |
| **5 Innovation** | 하네스 자동 진화 | ~$1,230/월 | 유지 및 개선 |

> 성숙도 Level 3 이상이면 Stripe 6계층 참조 아키텍처를 도입 기준으로 삼을 수 있다.
> 상세: [하네스 심화 §5, §7](claude-code-하네스-심화-아키텍처.md#5-참조-아키텍처-stripe-6계층-모델)
