...
6. [3 성능 축](#6-3-성능-축)
   - [Goal-Driven 실행 — 검증 가능한 성공 기준](#goal-driven-실행--검증-가능한-성공-기준)
7. [If-Then 규칙 엔진 패턴](#7-if-then-규칙-엔진-패턴)
8. [Vibe Review 패러다임](#8-vibe-review-패러다임)
9. [개발자 역할의 변화](#9-개발자-역할의-변화)
10. [사양 수집 패턴 — AskUserQuestion + SPEC.md](#10-사양-수집-패턴--askuserquestion--specmd)
...
- Stop Hooks — 응답 완료 시 결정론적 게이트 (합리화 방지, 품질 검증)

> **Stop hook 안전 제한**: 연속 8블록(8 consecutive Stop hook blocks) 호출 시 Claude Code가 턴을 강제 종료한다 (공식 Best Practices 수치). Stop hook 내 로직은 경량으로 유지해야 루프가 조기 종료되는 것을 방지할 수 있다.
...
새 세션을 열 때 바로 코드를 수정하지 않는다. 다음 4단계를 15분 안에 완료한 뒤 실행에 들어간다. — Anthropic 창시자 강연 권장 패턴

```
1. Done 정의 (3분)  → 완료 기준을 테스트/커맨드로 명시한다
                      예: "npm test 통과 + 빌드 성공"
2. 탐색 (7분)       → 수정 금지. 관련 파일 3~5개와 흐름만 파악
                      프롬프트: "코드 수정 금지, 파일 구조와 위험 포인트만 요약"
3. 1개 변경 (3분)   → 가장 안전한 단 하나의 변경만 실행
4. 검증 (2분)       → Done 기준으로 즉시 확인
```

> 이 루틴의 핵심은 "탐색 시 수정 금지"다. Claude에게 명시적으로 읽기 전용 탐색을 지시하지 않으면 탐색 중 임의 수정이 발생한다.
...
### Goal-Driven 실행 — 검증 가능한 성공 기준

> **Eval Coverage(축 3)** 는 하네스 전체의 검증 체계이고, **Goal-Driven 실행**은 개별 작업을 시작할 때 성공 기준을 미리 정의하는 원칙이다. 둘은 다른 레이어지만 같은 방향을 가리킨다: *기계로 확인 가능한 것을 기계가 확인하게 하라.*

LLM에게 "버그 고쳐줘"처럼 결과를 열어두면 Doom Loop(무한 재수정)가 생긴다. 검증 기준을 먼저 정의하면 AI가 독립적으로 반복하면서 목표에 도달할 수 있다.

| 약한 기준 (Doom Loop 유발) | 강한 기준 (Goal-Driven) |
|--------------------------|----------------------|
| "버그 고쳐줘" | "이 버그를 재현하는 테스트 작성 → 통과 확인" |
| "로그인 기능 추가해줘" | "E2E 테스트에서 유효/무효 자격증명 케이스 모두 통과" |
| "리팩터링해줘" | "기존 테스트 동일하게 통과 + 함수 30줄 이하" |
| "코드 작동하게 해줘" | "`npm test` 전체 green + lint 오류 0" |

**다단계 작업 계획 템플릿:**
```markdown
다음 단계로 진행해. 각 단계 완료 후 검증이 통과되면 다음으로 넘어가:

1. DB 스키마 마이그레이션 작성 → 검증: `npx prisma migrate dev` 오류 없음
2. API 엔드포인트 구현 → 검증: `curl -X POST ...` 200 응답 확인
3. 프론트엔드 연동 → 검증: 브라우저에서 폼 제출 후 성공 메시지 표시
```

→ 4원칙의 전체 맥락: [CLAUDE.md 실전 작성법 — Karpathy 4원칙 행동지침](./claude-code-CLAUDE-md-실전-작성법.md)
...
AI 시대의 개발자 리뷰 방식 전환. "vibe coding"을 대중화한 Andrej Karpathy의 관찰("LLM은 목표를 향해 반복하는 데 탁월하다")을 코드 리뷰 측면에 적용한 패러다임이다.
...
## 10. 사양 수집 패턴 — AskUserQuestion + SPEC.md

에이전트가 작업을 시작하기 전에 **요구사항을 구조적으로 수집**하는 패턴.
명확하지 않은 요구사항으로 시작하면 구현 중 방향 전환이 잦아지고 컨텍스트가 낭비된다.

### AskUserQuestion 인터뷰 메타프롬프트

Claude에게 AskUserQuestion 도구를 활용해 명세를 먼저 뽑아내도록 지시하는 메타프롬프트 패턴:

```markdown
<!-- CLAUDE.md 또는 작업 초반 프롬프트에 삽입 -->
## 작업 시작 전 인터뷰

새 기능 구현을 요청받으면 즉시 코드를 작성하지 말고, 먼저 아래 항목에 대해
AskUserQuestion으로 명세를 수집하라:

1. **성공 기준** — "어떻게 되면 완료인가?"
2. **제약 조건** — "건드리면 안 되는 것은 무엇인가?"
3. **현재 상태** — "지금 어떻게 동작하는가?"
4. **우선순위** — "가장 중요한 것 하나만 고른다면?"

수집 후 SPEC.md 초안을 작성하고 승인을 받은 뒤 구현을 시작하라.
```

### SPEC.md 패턴

작업마다 `.claude/SPEC.md`를 생성해 요구사항의 단일 진실 소스(Single Source of Truth)로 활용한다.
...
## 제약 조건
- 변경 금지: auth 미들웨어, DB 스키마
- 허용 범위: src/features/payment/

## 결정 보류 (명시적 미확정)
- 결제 실패 재시도 횟수: 미정
```

**효과**: 에이전트가 구현 중 방향을 잃었을 때 SPEC.md를 다시 참조해 재정렬할 수 있다.
불명확한 항목을 "결정 보류"로 명시하면 에이전트가 임의 가정을 하는 것을 방지한다.

### 요약 표 업데이트

| 개념 | 핵심 메시지 |
|------|------------|
| AskUserQuestion 인터뷰 | 코드 전에 명세. 4개 질문 → 착수 비용 절감 |
| SPEC.md | 단일 진실 소스. "결정 보류" 명시로 임의 가정 방지 |
